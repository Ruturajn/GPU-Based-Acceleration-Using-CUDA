...
__global__ void ImageHistKernel(unsigned char *d_lumin, unsigned char logmin,
                                unsigned char logmax,
                                int num_rows, int num_cols, unsigned int *d_hist,
                                unsigned char range_val, unsigned int *d_cdf)
{
...
void ImageHist(Mat &h_img, Mat &h_res)
{
    ...
    size_t channel_size = (h_img.rows * h_img.cols); //* sizeof(unsigned char));

    // Creating separate image for intensity channel
    Mat Y_chn(h_img.rows, h_img.cols, CV_8UC1),
        U_chn(h_img.rows, h_img.cols, CV_8UC1),
        V_chn(h_img.rows, h_img.cols, CV_8UC1);
    ...
    unsigned char _min = 0, _max = 0, range_val = 0;
    unsigned char *Y_chn_data = Y_chn.data;

    for (int i = 0; i < (h_img.rows * h_img.cols); i++)
    {
        if (Y_chn_data[i] < _min)
            _min = Y_chn_data[i];

        if (Y_chn_data[i] > _max)
            _max = Y_chn_data[i];
    }
    ...
    ImageHistKernel<<<blocks, threads>>>(d_lumin, _min, _max,
    ...
    // Do Tone mapping based on equalised histogram
        ...
        for (int i=0;i<(h_img.rows * h_img.cols);i++){
            int bin = (int)((num_bins * (Y_chn_data[i] - _min))/range_val);
            // if (Y_chn_data[i] == 0)
            //     cout << "bin = " << bin << endl;
    ...
    Mat Y_chn_eql(h_img.rows, h_img.cols, CV_8UC1, Y_chn_data);
