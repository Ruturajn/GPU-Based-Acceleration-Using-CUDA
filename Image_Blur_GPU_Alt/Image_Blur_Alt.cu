...
__global__ void ImgBlurKernel(unsigned char* d_img, int num_rows, int num_cols,
    ...
    // Making sure we don't access stuf out of the image
    if (row < num_rows && col < num_cols){
        ...
        for (int i=0;i<filter_width;i++){
            ...
            for (int j=0;j<filter_width;j++){
                ...
                float img_val_r = float(d_img[(index*3)]);
                float img_val_g = float(d_img[(index*3)+1]);
                float img_val_b = float(d_img[(index*3)+2]);
...
int main(void){
    ...
    cv::imwrite("../Images/Blurred_Output.png", h_res);
