...
__global__ void ImgBlurKernel(unsigned char* d_img, int num_rows, int num_cols,
                              int channels, float* d_filter, int filter_width, 
                              unsigned char* d_res){
        ...
        for (int k=0;k<channels;k++){
            ...
            for (int i=0;i<filter_width;i++){
                ...
                for (int j=0;j<filter_width;j++){
                    ...
                    // Defining Final row and column for the image
                    ...
                    float img_val = float(d_img[(index*channels) + k]);

                    // Calculate the filter value at that corresponding point
                    float filter_val = d_filter[(i*filter_width) + j];

                    // Perform convolution
                    prod += (img_val * filter_val);
                } 
            }
            // Writing result for each channel
            d_res[( ( (row * num_cols) + col) * channels) + k] = prod;
        }
    }
}
...
void ImgBlur(Mat& h_img, Mat& h_res, float* filter, int filter_width){
        ...
        ImgBlurKernel<<<blocks, threads>>>(d_frame, h_img.rows, h_img.cols, h_img.channels(),
...
int main(void){
    ...
    // Reading the image
    Mat h_img = imread("./cinque_terre_small.jpg", 1);
    ...
    Mat ref_img;
    GaussianBlur(h_img, ref_img, Size2i(kernel_width, kernel_width), 0);
    Mat diff_opencv = abs(ref_img - h_res);

    // Show the results
    imshow("Original", h_img);
    imshow("Blurred", h_res);
    ...
    imwrite("./Blurred_Output.png", h_res);
